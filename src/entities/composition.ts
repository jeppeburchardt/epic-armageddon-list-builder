...
export function deriveFormationUnits(entry: Entry, armyDef: ArmyDef): UnitTypeEntry[] {
    ...
    if (upgrade.type === 'replace') {
      ...
      if (upgradeDef && upgradeDef.type === 'replace') {
        ...
        if (existing) {
          ...
          const newInstances = existing.instances.slice(
            0,
            existing.instances.length - upgrade.replacedCount,
          )
        ...
        if (existingTo) {
          result.set(toName, {
            ...existingTo,
            instances: [...existingTo.instances, ...upgrade.replacingUnits.instances],
          })
        } else {
          result.set(toName, {
            ...upgrade.replacingUnits,
            instances: [...upgrade.replacingUnits.instances],
          })
        }
      }
      ...
      if (upgrade.chosenCharacterName) {
        ...
        if (charEntry) {
          result.set(upgrade.chosenCharacterName, {
            ...charEntry,
            instances: [...charEntry.instances, { weaponSelections: [] }],
          })
        } else {
          result.set(upgrade.chosenCharacterName, {
            unitName: upgrade.chosenCharacterName,
            instances: [{ weaponSelections: [] }],
          })
        }
      }
    } else {
      ...
      for (const ute of upgrade.addedUnits) {
        ...
        if (existing) {
          result.set(ute.unitName, {
            ...existing,
            instances: [...existing.instances, ...ute.instances],
          })
        } else {
  ...
  return Array.from(result.values()).filter((u) => u.instances.length > 0)
}
...
export function deriveUpgradeUnits(upgrade: AppliedUpgrade): UnitTypeEntry[] {
  ...
  if (upgrade.type === 'add') {
    ...
    if (upgrade.chosenCharacterName) {
      unitTypeEntries = [
        { unitName: upgrade.chosenCharacterName, instances: [{ weaponSelections: [] }] },
      ]
    }
  }
